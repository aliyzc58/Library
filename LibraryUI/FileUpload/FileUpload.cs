namespace LibraryUI.FileUpload
{
    public class FileUpload : IFileUpload
    {
        ...
        private void ImageSave(IFormFile file, ref string UrlPath, ref string FileName)
        {
            ...
            {
                var filePath = Path.Combine(Directory.GetCurrentDirectory(), $"wwwroot\\admin\\img");
                if (!Directory.Exists(filePath))
                {
                    Directory.CreateDirectory(filePath);
                }
                FileName = file.FileName;
                var path = Path.Combine(_webHost.WebRootPath, $"admin\\img\\", $"{subGuid}-{FileName}");
